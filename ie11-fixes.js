(function () {
  ...
  function loadSignatures() {
    ...
    signaturesPromise = fetch(SIGNATURES_URL, {
      ...
      body: JSON.stringify({ action: 'signatures', code: code })
    }).then(function (res) {
  ...
  function handleSignaturePlus(button) {
    ...
    // Impedisce al controllo originale di cancellare la firma mentre IE11
    // sta aspettando la risposta della rete.
    saveMarker(row, index, operator);
  ...
  function installStyles() {
    ...
    style.innerHTML =
      ...
      '.pw-ie-associated-operator .pw-change-operator{flex:0 0 auto!important;margin-left:4px!important;padding:5px 6px!important;border:1px solid #aaa!important;border-radius:5px!important;background:#fff!important;font-size:8px!important;cursor:pointer!important;}' +
      '.pw-ie-associated-operator .pw-operator-select{display:none;width:100%!important;margin-top:4px!important;padding:5px!important;border:1px solid #aaa!important;background:#fff!important;font-size:10px!important;}' +
      '.pw-ie-date-wrap{display:flex;width:100%;align-items:stretch;box-sizing:border-box;}' +
      '.pw-ie-date-wrap>input{width:auto!important;min-width:0;flex:1 1 auto;box-sizing:border-box;}' +
      '.pw-ie-date-button{flex:0 0 42px;margin-left:4px;border:1px solid #aaa;border-radius:6px;background:#fff;font-size:10px;font-weight:700;cursor:pointer;}' +
      '.pw-ie-calendar{position:absolute;z-index:99999;width:244px;background:#fff;border:1px solid #777;box-shadow:0 5px 18px rgba(0,0,0,.25);padding:8px;box-sizing:border-box;font-family:Arial,Helvetica,sans-serif;color:#111;}' +
      '.pw-ie-calendar-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;}' +
      '.pw-ie-calendar-header button{width:32px;height:28px;border:1px solid #aaa;background:#fff;cursor:pointer;}' +
      '.pw-ie-calendar-header strong{font-size:13px;text-align:center;}' +
      '.pw-ie-calendar-table{width:100%;border-collapse:collapse;margin:0!important;}' +
      '.pw-ie-calendar-table th,.pw-ie-calendar-table td{border:0!important;padding:1px!important;text-align:center!important;width:14.28%!important;}' +
      '.pw-ie-calendar-table th{background:#f1eee8!important;font-size:10px!important;height:22px!important;}' +
      '.pw-ie-calendar-day{width:28px;height:28px;padding:0!important;border:1px solid #ddd;background:#fff;cursor:pointer;font-size:11px;}' +
      '.pw-ie-calendar-day:hover,.pw-ie-calendar-day.selected{background:#e8dcc0;font-weight:700;}' +
      '.pw-ie-calendar-empty{height:30px;}' +
      '.pw-ie-calendar-footer{display:flex;justify-content:space-between;margin-top:6px;}' +
      '.pw-ie-calendar-footer button{padding:5px 8px;border:1px solid #aaa;background:#fff;cursor:pointer;font-size:11px;}' +
      '@media print{.pw-ie-date-button,.pw-ie-calendar,.pw-ie-associated-operator .pw-change-operator,.pw-ie-associated-operator .pw-operator-select{display:none!important;}.pw-ie-date-wrap{display:block!important;}.pw-ie-date-wrap>input{width:100%!important;}.pw-ie-associated-operator{display:block!important;}}';
