(() => {
  ...
  style.textContent = `
    ...
    @media(max-width:700px){.pw-office-grid{grid-template-columns:1fr}.pw-office-prep{margin-top:14px;padding:0 10px 24px}.pw-office-prep-card{padding:16px}.pw-office-actions button,.pw-ddt-actions button{width:100%;min-height:42px}}
  `;
  ...
  wrap.innerHTML = `
    <div class="pw-office-prep-card">
      ...
      <div class="pw-office-actions">
        <button type="button" class="pw-office-load" id="pwOfficeLoad">Carica commessa</button>
        <button type="button" class="pw-office-save" id="pwOfficeSave">Salva dati Ufficio</button>
      </div>
      <div class="pw-office-status" id="pwOfficeStatus"></div>
      <div class="pw-ddt-box">
        <div class="pw-ddt-title">DDT <span style="font-weight:400">(facoltativo)</span></div>
        <div class="pw-ddt-meta" id="pwDdtMeta">Nessun DDT allegato.</div>
        <div class="pw-ddt-actions" id="pwDdtActions">
          <button type="button" class="pw-ddt-add" id="pwDdtAdd">+ Allega DDT</button>
        </div>
        <input id="pwDdtFile" type="file" accept="application/pdf,image/*" hidden>
      </div>
    </div>`;
  ...
  async function call(body){
    const res=await fetch(API,{method:'POST',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify({...body,access_code:officeCode()})});
    let data={};try{data=await res.json()}catch(_){ }
    if(!res.ok)throw new Error(data?.error||`request_${res.status}`);
    return data;
  }
  function renderDdt(item){
    current=item||null;
    const name=String(item?.ddt_name||'');
    if(!name){
      ...
      ddtActions.querySelector('#pwDdtAdd').addEventListener('click',()=>fileEl.click());
      return;
    }
    const size=item?.ddt_size?` · ${(Number(item.ddt_size)/1024/1024).toFixed(2)} MB`:'';
    ddtMeta.textContent=`${name}${size}`;
    ...
    ddtActions.querySelector('.pw-ddt-replace').addEventListener('click',()=>fileEl.click());
  ...
  async function load(){
    ...
    commessaEl.value=c;setStatus('Caricamento…');
    try{
  ...
  async function uploadDdt(file){
    const c=commessa();if(!c){setStatus('Inserisci prima il numero di commessa.',true);fileEl.value='';return}
    if(!(file.type==='application/pdf'||file.type.startsWith('image/'))){setStatus('Il DDT deve essere un PDF o un’immagine.',true);fileEl.value='';return}
    if(file.size>15*1024*1024){setStatus('Il DDT supera il limite di 15 MB.',true);fileEl.value='';return}
  ...
  async function openDdt(){
    const c=commessa();if(!c)return;
    const popup=window.open('','_blank');
    try{
      const res=await fetch(API,{method:'POST',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'download_ddt',access_code:officeCode(),commessa:c,download:false})});
    ...
    }catch(err){if(popup)popup.close();console.error(err);setStatus('Non è stato possibile aprire il DDT.',true)}
  }
  ...
  wrap.querySelector('#pwDdtAdd').addEventListener('click',()=>fileEl.click());
  fileEl.addEventListener('change',()=>{const f=fileEl.files?.[0];if(f)uploadDdt(f)});
  commessaEl.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();load()}});
  commessaEl.addEventListener('input',()=>{clearTimeout(loadTimer);loadTimer=setTimeout(()=>{if(commessa().length>=2)load()},700)});
})();
